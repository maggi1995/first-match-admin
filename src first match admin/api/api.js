...
const refreshAuthLogic = async (failedRequest) => {
    ...
    console.log('from apis', userState,currentUser)
    if (!userState) {
...
export const saveClientConfigure = async (data, isSubmit) => {
    const currentUser = store.getState().loginData.user.token;
    console.log(" api submit", data, isSubmit)
    try {
        if (isSubmit === false) {
            return await axios
                .post(
                ...
                .then((response) => {
                    ...
                    console.log("save Client Configure from api.js", response)
                    return response.data;
                });
        }
        else {
            toast.error("please remove the '.' sign in Question names", {
...
function throwError(error) {
    if (error.response) {
        ...
        const errorResponse = {
            data: {
                Error: 'unknown error occurred while contacting the server'
            },
            status: undefined
        };
        throw errorResponse;
