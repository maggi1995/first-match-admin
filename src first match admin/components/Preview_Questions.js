...
import '../App.css';
import { wrap, subHeading, fieldRow, mainContent, twoCol, inputField, label1, fieldBox, fieldBox1, selectField, datePicker } from './Styles';
import { BrowserRouter as Router, Link } from 'react-router-dom';
import { ConfigIcon, NewClientIcon, ExistingClientIcon } from './icons';
...
export class Preview extends React.Component {
        ...
        this.props.DynamicQuestions.map(
            (sec) =>
                ...
                sec.questions.map((ques) => {
                    client_form.push({
                        [ques.question.replace(/ /g, '_')]: Array.isArray(ques.answer)
                            ? ques.suggested_answers.map((q, j) => ques.answer.includes(q.value) && q.id.toString()).filter((item) => item !== false)
                            : ques.answer === 0
                            ? ques.answer.toString()
                            : ques.suggested_answers.length >= 1
                            ? ques.answer && ques.suggested_answers.filter((p, i) => p.value === ques.answer)[0].id.toString()
                            : ques.answer
                            ? ques.answer.toString()
                            : ''
                    });
    ...
    handleChange = async (e) => {
        ...
        if (type === 'select') {
            ...
            DynamicQuestions.map((sec, i) =>
                jumpto.includes(sec.section)
                    ? (DynamicQuestions[i].related = 'false')
                    : this.state.prevJump[name.replace(/ /g, '_')] && this.state.prevJump[name.replace(/ /g, '_')].includes(sec.section) && (DynamicQuestions[i].related = 'true')
            );
        } else {
            if (type === 'radio') {
            ...
            if (type === 'checkbox') {
                ...
                this.setState({
                    client_form: {
                        ...
                        [name]: this.state.client_form[name] ? (checked ? this.state.client_form[name].concat([value]) : this.state.client_form[name].filter((idy) => idy !== value)) : [value]
                    },
                    hasError: false
                });
            } else {
    ...
    handleSubmit = async (e) => {
        ...
        Object.keys(client_form).map((ele, i) => (data.push({ [ele.replace(/_/g, ' ')]: client_form[ele] }), !client_form[ele] && Required_List[ele] === 'yes' && (isValid_Data = false)));
    ...
    render() {
        ...
        return (
            <Fragment>
                <div style={logo}>
                    <img style={firstMatchLogo} alt="Logo" src={`${baseApiUrl}/${logoPath}`} />
                </div>
                <div style={nav}>
                ...
                <div style={wrap}>
                    <div style={mainContent}>
                        ...
                        <Modal isOpen={this.state.isOpen} ariaHideApp={false} onRequestClose={this.handleClose} style={customStyles} contentLabel="Example Modal">
                            <div>
                                                                    ...
                                                                    {ques.answer_type === 'SELECT' ? (
                                                                        ...
                                                                        >
                                                                            ...
                                                                            {ques.suggested_answers.map((ans, i) => (
                                                                                <option
                                                                                    ...
                                                                                    data-idx={index}
                                                                                    data-idy={ind}
                                                                                    data-jump={ques.suggested_jump.map((sj) =>
                                                                                        sj !== null && ans.value === sj.answer ? (sj.jumpto ? sj.jumpto : '') : ''
                                                                                    )}
                                                                                    data-quesjump={
                                                                                        ques.suggested_jump.length > 0
                                                                                            ? ques.suggested_jump.map((sj) =>
                                                                                                  sj !== null && ans.value === sj.answer ? (sj.question_jumpto ? sj.question_jumpto : '') : ''
                                                                                              )
                                                                                            : ''
                                                                                    }
                                                                                    selected={
                                                                                        this.state.client_form[ques.question.replace(/ /g, '_')] &&
                                                                                        this.state.client_form[ques.question.replace(/ /g, '_')].toString() === ans.id.toString()
                                                                                    }
                                                                                >
                                                                                    {ans.value}
                                                                                </option>
                                                                                ...
                                                                                >
                                                                                    <React.Fragment>
                                                                                        <input
                                                                                            ...
                                                                                            value={ans.id}
                                                                                            checked={
                                                                                                this.state.client_form[ques.question.replace(/ /g, '_')] === ans.id.toString()
                                                                                                    ? true
                                                                            ...
                                                                            >
                                                                                <React.Fragment>
                                                                                    <input
                                                                                        ...
                                                                                        checked={
                                                                                            (this.state.client_form[ques.question.replace(/ /g, '_')] &&
                                                                                                this.state.client_form[ques.question.replace(/ /g, '_')].includes(ans.id.toString())) ||
                                                                                            (this.state.client_form[ques.question.replace(/ /g, '_')] &&
                                                                                                this.state.client_form[ques.question.replace(/ /g, '_')].includes(ans.value))
                                                                                        }
                                                                                    />{' '}
                                                                        ...
                                                                        <Fragment>
                                                                            <input
                                                                                ...
                                                                                value={
                                                                                    ques.question === 'Age'
                                                                                        ? (this.state.client_form[ques.question.replace(/ /g, '_')] = this.getAge(
                                                                                              this.state.client_form['Date_of_Birth'],
                                                                                              this.state.client_form['Date_of_Referral']
                                                                                          ))
                                                                                        : this.state.client_form[ques.question.replace(/ /g, '_')]
                                                                                }
