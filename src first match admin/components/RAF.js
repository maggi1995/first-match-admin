...
class RAF extends Component {
            ...
            sections.map((questions) =>
                questions.questions.map((question) => {
                    question.customer_answers.length > 0
                        ? question.customer_answers.map((answers) => {
                              this.setState({
                                  data: {
                                      ...this.state.data,
                                      [question.id]:
                                          question.question_type === 'FILE' ? (this.state.data[question.id] ? this.state.data[question.id].concat([answers.answer]) : [answers.answer]) : answers.answer
                                  },
                                  addFile: {
                                      ...this.state.addFile,
    ...
    onChange = async (e) => {
        ...
        if (file) {
            ...
            this.state.data[name] && this.state.data[name][index] ? this.updateData(index, name, file_path, Error_Msg) : this.updateData1(name, file_path, Error_Msg);
        } else {
            let message = this.state.uploadMsg[name];
            message[index] = [''];
            let data = this.state.data[name];
    ...
    handleChange = (e) => {
        ...
        if (type) {
            ...
            loadrafquestions[id].questions.map((ques, i) =>
                ques.question === jump
                    ? (loadrafquestions[id].questions[i].is_related = 'false')
                    : ques.question === this.state.prevJump[name] && (loadrafquestions[id].questions[i].is_related = 'true')
            );
            this.setState({
                prevJump: {
                    [name]: jump
    ...
    handleCheckboxChange = (e) => {
        ...
        const loadrafquestions = this.state.loadrafquestions.response;
        loadrafquestions[id].questions.map((ques, i) =>
            ques.question === jump ? (loadrafquestions[id].questions[i].is_related = 'false') : ques.question === this.state.prevJump[name] && (loadrafquestions[id].questions[i].is_related = 'true')
        );
        this.setState({
            prevJump: {
                [name]: jump
        ...
        try {
            await submitRAF(newArray, customer, is_completely_filled);
            {
                is_completely_filled === 'true'
                    ? this.props.history.push('/admin/rafmsg')
    ...
    deleteFile = (i) => (e) => {
        e.preventDefault();
        let id = e.target.dataset.id;
        let data = this.state.data[id] ? [...this.state.data[id].slice(0, i), ...this.state.data[id].slice(i + 1)] : [];
        let uploadMsg = this.state.uploadMsg[id] ? [...this.state.uploadMsg[id].slice(0, i), ...this.state.uploadMsg[id].slice(i + 1)] : [];
        let addFile = [...this.state.addFile[id].slice(0, i), ...this.state.addFile[id].slice(i + 1)];
        this.setState({
            addFile: {
                ...this.state.addFile,
                [id]: addFile
    ...
    display = () => {
        const display =
            this.state.loadrafquestions.response &&
            this.state.loadrafquestions.response.map((data, id) => (
                <>
                    <div key={id} className="row p-2 mb-3 bg-primary text-white">
                        {data.title}
                    </div>
                    {data.questions.map((quest, idx) =>
                        quest.is_related === 'true' ? (
                            ...
                            <>
                                <div key={idx} className="form-group">
                                    ...
                                    <input
                                        ...
                                        data-id={id}
                                        name={quest.id}
                                        value={this.state.data[quest.id]}
                            ...
                            <>
                                <label key={idx} className="font-weight-bold">
                                    {quest.question}
                                </label>
                                {Object.keys(this.state.addFile[quest.id]).map((a, index) => (
                                    <div className="form-group ml-4">
                                        ...
                                        {this.state.addFile[quest.id].length - 1 === index && (
                                            <i className="fa fa-plus" onClick={this.addFiles(index)} style={{ fontSize: '24px' }} data-id={quest.id}></i>
                                        )}
                                        &nbsp;
                                        {index > 0 && (
                                            <i
                                        ...
                                        {this.state.uploadMsg[quest.id] && this.state.uploadMsg[quest.id][index] ? (
                                            ...
                                            <>
                                                {this.state.data[quest.id] && this.state.data[quest.id][index] && (
                            ...
                            <>
                                ...
                                <div className="form-group ml-4">
                                    <select name={quest.id} data-type="select" className="form-control col-5" id="exampleFormControlSelect1" onChange={this.handleChange}>
                                        ...
                                        {quest.suggested_answers.map((suggested, idy) => (
                                            <option key={idy} data-jump={quest.suggested_jump[idy]} data-id={id} value={suggested} selected={this.state.data[quest.id] === suggested}>
                                                {suggested}
                                            </option>
                                        ))}
                                    </select>
        ...
        return (
            <div className="cotainer-fluid">
                <div className="text-center">
                    <h2>Readiness Assessment FORM</h2>
                </div>
                ...
                <div className="card-body shadow-sm p-3 m-3 bg-white rounded">
                    {this.state.error ? (
                        <div className="col-sm-12 d-flex shadow-lg p-3 mb-5 bg-white rounded justify-content-center text-info font-weight-bold h2">{this.state.error}</div>
                    ) : (
                        <>
                            {loading ? (
                                ''
                                    ...
                                    <fieldset className="form-group">
                                        ...
                                        <div className="form-group row d-flex justify-content-center">
                                            ...
                                            <div className="col-sm-6 d-flex justify-content-center">
                                                <button type="submit" className="btn btn-primary btn btn-block" data-id="submit" onClick={this.handleShow}>
                                                    Submit
                                                </button>
                                            </div>
                                            <div className="col-sm-3  ">&nbsp;</div>
                    ...
                    <Modal isOpen={this.state.showPOPUP} onRequestClose={this.handleClose} style={customStyles} contentLabel="Forgot Password" ariaHideApp={false}>
                        <p className="text-center">Are you sure to submit the data ? Once submitted this link cannot be accessible</p>

                        <div className="row ">
                            <div className="col text-center ">
                                <button className="button-pop" data-id={this.state.btnAction} onClick={this.handleSubmit}>
                                    Yes
                                </button>
                            </div>
                            <div className="col text-center ">
