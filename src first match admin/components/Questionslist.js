...
export class QuestionsList extends React.Component {
    ...
    handleChange = async (e) => {
        ...
        const subCat = this.state.QuestionsCategory.response && this.state.QuestionsCategory.response.filter((data, idx) => (data.children.length > 0 ? data.id == value : null));
    ...
    category = () => {
        const category = this.state.QuestionsCategory.response && this.state.QuestionsCategory.response.map((data, idx) => <option value={data.id}>{data.title}</option>);
        return category;
    };
    subcategory = () => {
        const subcategory =
        ...
        return (
            <div className="container-fluid">
                <div className="row p-2 bg-primary text-white">
                    <span className="ml-4">Questions List</span>
                </div>
                ...
                <Modal isOpen={this.state.showPOPUP} onRequestClose={this.handleClose} style={customStyles} contentLabel="Forgot Password" ariaHideApp={false}>
                    ...
                    <div className="row ">
                        <div className="col-6 text-center ">
                            <button className="button-pop" data-id={this.state.btnAction} onClick={this.state.btnAction ? this.handleDelete : this.handleMove}>
                                Yes
                            </button>
                        </div>
                        <div className="col-6 text-center ">
