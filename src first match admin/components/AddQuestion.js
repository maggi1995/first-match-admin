...
class AddQuestion extends React.Component {
    ...
    handleDelete = (i) => (e) => {
        e.preventDefault();
        let suggested_answers = [...this.state.suggested_answers.slice(0, i), ...this.state.suggested_answers.slice(i + 1)];
        this.setState({
            suggested_answers
        });
    };
    ...
    display = () => {
        const display =
            ...
            this.props.questionscategoryList.response.map((data, id) => (
                <>
                    <div key={id} className="form-check">
                        <input className="form-check-input" onChange={this.onRadioChange} value={data.id} type="radio" name="category" checked={this.state.category == data.id} required />
                    ...
                    {data.children.length >= 1
                        ? data.children.map((child, i) => (
                              <div key={i} className="form-check form-check-inline mr-4 ml-4">
                                  <input className="form-check-input" onChange={this.onRadioChange} value={child.id} type="radio" name="category" required checked={this.state.category == child.id} />
                                  <label className="form-check-label">{child.title}</label>
                              </div>
                          ))
                        : ''}
                </>
        ...
        return (
            <div className="container-fluid">
                <form className="" onSubmit={this.addQues}>
                    <div className="row p-2 bg-primary text-white">{id ? 'Edit' : 'Add New'} Question</div>
                    <br />
                    ...
                    <fieldset className="form-group">
                        <div className="row">
                            ...
                            <div className="col-sm-10">{this.display()}</div>
                        </div>
                    </fieldset>
                    <div className="form-group row">
                        <label className="col-sm-2 col-form-label font-weight-bold ">Question:</label>
                        <div className="col-sm-5">
                            <input type="text" onChange={this.handleChange} value={this.state.question} name="question" className="form-control " id="inputEmail3" required />
                        </div>
                    </div>
                    <fieldset className="form-group">
                        <div className="row">
                            ...
                            <div className="col-sm-10">
                                <div className="form-check form-check-inline">
                                    <input
                                        ...
                                        name="question_type"
                                        type="radio"
                                        checked={this.state.question_type === 'SELECT'}
                                ...
                                <div className="form-check form-check-inline">
                                    <input
                                        ...
                                        type="radio"
                                        value="TEXT"
                                        disabled={id && this.state.question_type !== 'TEXT' ? true : false}
                                        required
                                    />
                    ...
                    {this.state.question_type === 'FILE' || this.state.question_type === 'TEXT' || this.state.question_type === ''
                        ? ''
                        : this.state.suggested_answers.map((question, index) => (
                              <div key={index} className="form-group row">
                                  <label className="col-sm-2 col-form-label font-weight-bold">
                                      Answer {index + 1}:{question.answer}
                                  </label>
                                  <div className="col-sm-3">
                                      <input type="text" onChange={this.handleText(index)} value={question} className="form-control" required />
                                      {index !== 0 && (
                                          <div
                                              style={{
                                                  position: 'absolute',
                                      ...
                                      {this.state.question_type === 'FILE' || this.state.question_type === 'TEXT'
                                          ? ''
                                          : index === this.state.suggested_answers.length - 1 && (
                                                <div
                                                    style={{
                                                        position: 'absolute',
                                                        top: '1px',
                                  ...
                                  <>
                                      ...
                                      <div className="col-sm-3">
                                          <select name="jumpto" className="form-control" id="exampleFormControlSelect1" data-name="suggested_jump" onChange={this.handleJump(index)}>
                                              <option value="">Select</option>
                                              {relatedQuestions
                                                  .filter((sec, key) => sec.is_related === true)
                                                  .map((ques, i) => (
                    ...
                    {this.state.question_type === 'TEXT' ? (
                        <div className="form-group row">
                            ...
                            <div className="col-sm-3">
                                <select name="jumpto" className="form-control" id="exampleFormControlSelect1" data-name="suggested_jump" onChange={this.handleJump()}>
                                    <option value="">Select</option>
                                    {relatedQuestions.map(
                                        (sec, key) =>
                                            sec.is_related === true && (
                                                <option key={key} value={sec.question} selected={sec.question === this.state.suggested_jump}>
                                                    {sec.question}
                                                </option>
                                            )
                                    )}
                    ...
                    <div className="form-check form-check">
                        <input type="checkbox" className="form-check-input" name="is_related" value={this.state.is_related} checked={this.state.is_related === true} onChange={this.onRadioChange} />
